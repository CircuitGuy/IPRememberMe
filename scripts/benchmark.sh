...
# Env:
#   TARGET_RUNTIME_SECONDS (default 10) - run each mode until runtime exceeds this value (bursts of BURST_REQUESTS)
#   BURST_REQUESTS (default 100)        - number of requests per burst
...
#   CURL_TIMEOUT (default 0.1)     - per-request timeout in seconds
...
TARGET_RUNTIME_SECONDS=${TARGET_RUNTIME_SECONDS:-10}
...
measure() {
  ...
  total_req=0
  start_s=$(date +%s)
  >&2 printf "Running %s bursts (>= %ss)\n" "$label" "$TARGET_RUNTIME_SECONDS"
  while true; do
    for _ in $(seq 1 "$BURST_REQUESTS"); do
      if ! t=$(run_curl "$ca_opt" "$url"); then
        if [ "$timeout_hit" -eq 0 ]; then
          timeout_hit=1
          >&2 printf "\n"
          echo "$label measurement timed out (per-request timeout ${CURL_TIMEOUT}s)"
        fi
        rm -f "$tmp"
        return 1
      fi
    ...
    current_s=$(date +%s)
    elapsed_s=$((current_s - start_s))
    if [ "$elapsed_s" -ge "$TARGET_RUNTIME_SECONDS" ]; then
      break
    fi
    >&2 printf "."
  done
  >&2 printf "\n"
  summary=$(summarize_file "$tmp" "$label")
  rm -f "$tmp"
  end_s=$(date +%s)
  elapsed_s=$((end_s - start_s))
  echo "$summary"
  echo "$label totals: count=$total_req runtime=${elapsed_s}s"
  return 0
}
...
echo "Settings: targetRuntime=${TARGET_RUNTIME_SECONDS}s burst=$BURST_REQUESTS timeout=${CURL_TIMEOUT}s use_docker_curl=$USE_DOCKER_CURL"
start_ts=$(date +%s)
...
direct_out=$(measure "DIRECT" "$direct_url" || echo "")
...
ip_out=$(measure "IPREMEMBER" "$ip_url" || echo "")
  if [ -z "$ip_out" ]; then
