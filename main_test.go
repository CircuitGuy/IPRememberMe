...
func TestStoreRefreshRequiresExistingEntry(t *testing.T) {
	s := newStore(time.Hour, 2)
	now := time.Now()
	if _, ok := s.refresh("1.2.3.4", "alice", now); ok {
		t.Fatalf("expected refresh to fail without entry")
	}
	if _, err := s.upsert("1.2.3.4", "alice", now); err != nil {
		t.Fatalf("unexpected upsert err: %v", err)
	}
	if _, ok := s.refresh("1.2.3.4", "alice", now.Add(5*time.Minute)); !ok {
		t.Fatalf("expected refresh to succeed after entry exists")
	}
}
...
func TestCookieDoesNotRecreateAfterClear(t *testing.T) {
	s := newTestServer(t)
	ip := "4.3.2.1"
	user := "bob"
	now := time.Now()
	if _, err := s.store.upsert(ip, user, now); err != nil {
		t.Fatalf("initial upsert: %v", err)
	}
	token := signToken([]byte(s.cfg.SharedSecret), ip, user, now.Add(time.Hour))
	req := httptest.NewRequest(http.MethodGet, "/status", nil)
	req.RemoteAddr = ip + ":4567"
	req.Header.Set("Cookie", cookieName+"="+token)

	if _, ok := s.maybeRefreshFromCookie(httptest.NewRecorder(), req, ip, now.Add(2*time.Minute)); !ok {
		t.Fatalf("expected cookie refresh to succeed while entry exists")
	}

	s.store.clear(ip)

	if _, ok := s.maybeRefreshFromCookie(httptest.NewRecorder(), req, ip, now.Add(4*time.Minute)); ok {
		t.Fatalf("expected cookie refresh to fail after entry cleared")
	}
}
...
func TestSetCookieHonorsConfiguredDomain(t *testing.T) {
	cfg := config{
		SharedSecret:    "secret",
		Duration:        time.Hour,
		MaxIPsPerUser:   3,
	...
	if found.Domain != "example.com" {
		t.Fatalf("expected configured domain example.com, got %q", found.Domain)
	}
}

func TestSetCookieDomainOverridesHost(t *testing.T) {
	cfg := config{
		SharedSecret:    "secret",
		Duration:        time.Hour,
		MaxIPsPerUser:   3,
	...
	rr := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodGet, "/dummy", nil)
	req.Host = "other.test:8443"
	req.Header.Set("X-Forwarded-Host", "other.test:8443")
	s.setCookie(rr, req, "1.1.1.1", "alice", time.Now().Add(time.Hour))
	...
	if found.Domain != "example.com" {
		t.Fatalf("expected configured domain example.com even with different host, got %q", found.Domain)
	}
}

func TestSetCookieHostOnlyWhenDomainUnset(t *testing.T) {
	s := newTestServer(t)
	rr := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodGet, "/dummy", nil)
	s.setCookie(rr, req, "1.1.1.1", "alice", time.Now().Add(time.Hour))
	...
	if found.Domain != "" {
		t.Fatalf("expected host-only cookie with empty domain, got %q", found.Domain)
	}
}
...
func TestAdminEndpointsAuthorized(t *testing.T) {
	...
	// Method enforcement
	reqClearGet := httptest.NewRequest(http.MethodGet, "/admin/clear", nil)
	reqClearGet.Header.Set("Authorization", "Bearer "+s.cfg.SharedSecret)
	rrClearGet := httptest.NewRecorder()
	s.handleAdminClear(rrClearGet, reqClearGet)
	if rrClearGet.Code != http.StatusMethodNotAllowed {
		t.Fatalf("expected 405 for GET clear, got %d", rrClearGet.Code)
	}
}
...
func TestRejectsMultipleForwardedFor(t *testing.T) {
	s := newTestServer(t)
	req := httptest.NewRequest(http.MethodGet, "/status", nil)
	...
	if rr.Code != http.StatusBadRequest {
		t.Fatalf("expected 400 for multiple forwarded-for entries, got %d", rr.Code)
	}
}
