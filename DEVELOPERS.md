...
- Container security lint: `./scripts/container-scan.sh` (builds the image with `IPREMEMBER_IMAGE`, runs hadolint on the Dockerfile, Trivy scan defaulting to `CRITICAL` severity with `--ignore-unfixed`, then Dockle checks). Tune via `TRIVY_SEVERITY`/`TRIVY_IGNORE_UNFIXED`/`DOCKLE_EXIT_LEVEL`; hadolint ignores DL3007 by default because Chainguard only publishes `latest` for the builder (override with `HADOLINT_ARGS`). Invoked automatically by the stack scripts and CI; requires Docker + network access for vulnerability DBs.
